    ...
    def dbassert(self,code,res_result,data):
        ...
        if code =='200' :
            ...
            for j in range(len(data_result)):
                self.assertEqual(data_result[j][0],res_result['data']['CARDID'],msg="cardid不相等")
                self.assertEqual(data_result[j][1],res_result['data']['VIPINFOID'],msg="vipinfoid")
                self.assertEqual(data_result[j][2],res_result['data']['VIPNAME'],msg="VIPNAME")
                #self.assertEqual(data_result[j][3],res_result['data']['data']['SEX'],msg="SEX")
                self.assertEqual(data_result[j][4],res_result['data']['MOBILETEL'],msg="MOBILETEL")
                #self.assertEqual(data_result[j][5],res_result['data']['data']['REMARK'],msg="REMARK")
                self.assertEqual(data_result[j][6],res_result['data']['PUTOUTMODE'],msg="PUTOUTMODE")
                self.assertEqual(data_result[j][7],res_result['data']['SHOPCODE'],msg="SHOPCODE")
                self.assertEqual(data_result[j][8],res_result['data']['OPERATOR'],msg="OPERATOR")
                #self.assertEqual(data_result[j][9],res_result['data']['data']['CARDLEVEL'],msg="会员级别")
                self.assertEqual(data_result[j][10],res_result['data']['STATUS'],msg="会员状态")
                #self.assertEqual(data_result[j][11],res_result['data']['data']['FLYMAIL'],msg="微信")     
                self.assertEqual(data_result[j][12],res_result['data']['INTEGRAL'],msg="积分当前")
                self.assertEqual(data_result[j][13],res_result['data']['TOTALINTEGRAL'],msg="累计积分") 
                self.assertEqual(data_result[j][14],res_result['data']['VIPCARDTYPE'],msg="vip类型")
                self.assertEqual(data_result[j][15],res_result['data']['VIPID'],msg="vipid")        
            ...
            for j in range(len(data_result)):
                ...
                self.assertEqual(data_result[j][11],None,msg="当前积分")            
